<section id="faqs" class="max-w-[1200px] w-full mx-auto mb-[140px] relative px-6">
   <div class="mb-12 max-w-[460px] mx-auto relative border border-dashed border-gray-500">
      @include('components.icons.edit-pen', [ 'class'=>'w-8 h-8', 'dialog' => 'faqSection'])
   ...
   <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center relative">
      <div class="flex flex-col gap-6 relative" id="accordionExample">
         ...
         @foreach ($faqs_content->child_sections as $index => $faq)
            <div
               class="relative z-10 rounded-lg border dark:border-gray-700 bg-white dark:bg-gray-900 border-dashed"
            >
               @include('components.icons.edit-pen', 
                  [ 'class'=>'w-6 h-6', 'dialog' => "faqNo$faq->id"]
               )
...
@include('components.customize.edit_section', 
    ['dialog' => 'faqSection', 'section' => $faqs_content]
)

@include('components.customize.edit_child_section', 
   [
      'dialog' => "faqNo".$faqs_content->child_sections[0]->id, 
      'title' => "FAQs Section".$faqs_content->child_sections[0]->id, 
      'section' => $faqs_content->child_sections[0]
   ]
)
@include('components.customize.edit_child_section', 
   [
      'dialog' => "faqNo".$faqs_content->child_sections[1]->id, 
      'title' => "FAQs Section".$faqs_content->child_sections[1]->id, 
      'section' => $faqs_content->child_sections[1]
   ]
)
@include('components.customize.edit_child_section', 
   [
      'dialog' => "faqNo".$faqs_content->child_sections[2]->id, 
      'title' => "FAQs Section".$faqs_content->child_sections[2]->id, 
      'section' => $faqs_content->child_sections[2]
   ]
)
@include('components.customize.edit_child_section', 
   [
      'dialog' => "faqNo".$faqs_content->child_sections[3]->id, 
      'title' => "FAQs Section".$faqs_content->child_sections[3]->id, 
      'section' => $faqs_content->child_sections[3]
   ]
)
@include('components.customize.edit_child_section', 
   [
      'dialog' => "faqNo".$faqs_content->child_sections[4]->id, 
      'title' => "FAQs Section".$faqs_content->child_sections[4]->id, 
      'section' => $faqs_content->child_sections[4]
   ]
)