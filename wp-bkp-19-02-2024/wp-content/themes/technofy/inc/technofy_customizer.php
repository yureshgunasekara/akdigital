...
class technofy_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'technofy_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'technofy-customizer-style', get_template_directory_uri() . '/inc/css/technofy-customizer-style.css',array());
			wp_enqueue_script( 'technofy-customizer', get_template_directory_uri() . '/inc/js/technofy-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function technofy_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new Technofy_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function technofy_data_load() {

		$data = apply_filters( 'technofy_customizer_data', array());
...
add_action('init','technofy_customizer::technofy_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class Technofy_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','technofy'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','technofy'); ?></span>  
